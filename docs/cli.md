...
| Command | Arguments | Behavior |
|---------|-----------|----------|
| `normalize-target github` | `--metadata-before FILE`, `--metadata-after FILE`, `--base-archive FILE`, `--head-archive FILE`, `--output FILE`, `--diff-output FILE` | Normalize a GitHub PR into a `github-pr` manifest + **derived** exact patch **without** mutating a run. Consumes the `gh pr view --json …` documents captured **before and after** fetching the base/head snapshots; every immutable identity field (number/url/title/body/files, base/head repository + `baseRefOid`/`headRefOid`, `isCrossRepository`) must match between the two reads or the target is rejected (retry the acquisition). The patch is **derived** from the `--base-archive` (base `repository@baseRefOid`) and `--head-archive` (head `repository@headRefOid`) codeload snapshots — never `gh pr diff` or a caller-supplied patch — and `changed_files` is derived from it and must agree with the metadata file list. Branch names are display metadata; OIDs and repository identities are authoritative. |
