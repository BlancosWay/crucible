...
A thread (and the plan, and the final review) **loops**: both peers investigate → one peer assembles
the candidate finding set → **both peers independently attest to it** → repeat. Consensus is decided
...
- **Gate progress is decided from peer objections**, never from an accepted candidate finding's
  severity. An **objection** is a defect in the candidate; a **candidate finding** is a result the
  peers accept. So a candidate that *accepts a blocker* still reaches `CONSENSUS` when both peers
...
- Because both peers attest **every** round, reaching `CONSENSUS` means both peers — each having
  ...
  This holds even for a one-round thread: consensus can never occur after only one peer attested.
...
reviewed; the accepted plan/thread-graph and each reviewed thread are immutable thereafter (a change
...
deep dive uses `crucible symmetric-verdict`, which takes **no** `--resolutions` — clearing a blocking
